...
import { AlertTriangle, ArrowLeft, ArrowRight, BookOpenCheck, Bug, Check, CircleHelp, ClipboardCheck, FileBarChart2, Lightbulb, ListChecks, Maximize2, Minimize2, MousePointer2, Music2, QrCode, RotateCcw, Search, ShieldCheck, Sparkles, Store, UtensilsCrossed, X } from "lucide-react";
...
export default function OnboardingCenter({ role, compact = false }: { role: Role; compact?: boolean }) {
  ...
  const matchingSolutions = useMemo(() => {
    ...
    return roleSolutions.filter(solution => [solution.title, solution.description, ...solution.actions, ...solution.keywords].join(" ").toLocaleLowerCase("es").includes(query));
  }, [helpQuery, role]);
